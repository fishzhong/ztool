...
func Run() error {
	...
	//处理推送消息以及事件
	r.Any("/api/v1/oa/serve", exampleOffAccount.Serve)
	//获取ak
	...
	//获取
	r.GET("api/v1/oa/template", exampleOffAccount.Template) //发送模板消息
	r.GET("api/v1/oa/redirect", exampleOffAccount.Redirect)
	r.GET("api/v1/oa/token", exampleOffAccount.AccToken)
	r.GET("api/v1/oa/ticket", exampleOffAccount.GetQrTicket)
